function [StatisticalPathway,SpectralDepthTable]=SCALES(OrganismOfInterest,ProteinOfInterest,V_protein_matrices,Proteomes)
...
R_21_121=corr((V_protein_matrices.(OrganismOfInterest)(:,21:121))');



i_StatisticalPathway=find(abs(R_21_121(:,(strcmp(Proteomes.(OrganismOfInterest).Genes.UniprotIDs(:,1),ProteinOfInterest))))>threshold);
StatisticalPathway=Proteomes.(OrganismOfInterest).Genes.GeneNames(i_StatisticalPathway);
...
for i=1:1:length(centers)
    R_StatisticalPathway(:,:,i)=corr((V_protein_matrices.(OrganismOfInterest)(i_StatisticalPathway,centers(i)-window/2:centers(i)+window/2))');
...
% yticklabels(Proteomes.(OrganismOfInterest).Genes.GeneNames(i_StatisticalPathway(fliplr(outperm_depth))));
% xticklabels(Proteomes.(OrganismOfInterest).Genes.GeneNames(i_StatisticalPathway(fliplr(outperm_depth))));
...
RowColNames={};
for i=1:1:length(i_StatisticalPathway)
    RowColNames{i}=[Proteomes.(OrganismOfInterest).Genes.UniprotIDs{i_StatisticalPathway(i)} '_' Proteomes.(OrganismOfInterest).Genes.GeneNames{i_StatisticalPathway(i)}];
end
RowColNames
